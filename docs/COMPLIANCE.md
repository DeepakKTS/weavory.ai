...
| Hash-chain tamper detection | `pnpm exec tsx examples/wall_incident.ts` (tampers an entry, alarm fires, incident file is exported) |
| Bi-temporal recall | `pnpm exec tsx examples/gauntlet_rewind.ts` (forget + `as_of` replay) |
| Trust gating / adversarial mode | `pnpm exec tsx examples/wall_adversarial.ts` |
